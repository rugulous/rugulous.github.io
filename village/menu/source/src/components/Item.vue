...
<template>
	<div class="mb-3" :class="{ 'no-stock': !isInStock(item) }">
		...
		<small class="d-block mb-2">Contains {{ formatIngredients(item.ingredients) }}</small>


		<p class="m-0" v-for="variant in item.variants"><strong>{{ variant.name }}</strong> <em>{{ variant.price }}</em></p>
